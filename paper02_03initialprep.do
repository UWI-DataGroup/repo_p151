** HEADER -----------------------------------------------------
**  DO-FILE METADATA
    //  algorithm name					paper02_03initialprep.do
    //  project:				        
    ...
    ** Close any open log file and open a new log file
    capture log close
    log using "`logpath'\paper02_03initialprep", replace
** HEADER -----------------------------------------------------

** RUN covidprofiles_002_jhopkins.do BEFORE this algorithm
use "`datapath'\version02\2-working\paper02_jhopkins", clear 
...
** UK, US, Germany, Italy, Sweden, Brazil, Chile, Mexico, Peru are made up of groups of territories. Collapse and sum
...
/// Latin America
/// Central American Countries
replace iso = "CRI" if countryregion=="Costa Rica" 
replace iso = "SLV" if countryregion=="El Salvador" 
replace iso = "GTM" if countryregion=="Guatemala" 
replace iso = "HND" if countryregion=="Honduras" 
replace iso = "MEX" if countryregion=="Mexico" 
replace iso = "NIC" if countryregion=="Nicaragua"
replace iso = "PAN" if countryregion=="Panama" 
/// South American Countries
replace iso = "ARG" if countryregion=="Argentina" 
replace iso = "BOL" if countryregion=="Bolivia" 
replace iso = "BRA" if countryregion=="Brazil" 
replace iso = "CHL" if countryregion=="Chile" 
replace iso = "COL" if countryregion=="Colombia" 
replace iso = "ECU" if countryregion=="Ecuador" 
replace iso = "PRY" if countryregion=="Paraguay"
replace iso = "PER" if countryregion=="Peru" 
replace iso = "URY" if countryregion=="Uruguay"
replace iso = "VEN" if countryregion=="Venezuela"
...
save "`datapath'\version02\2-working\paper02_jhopkins_clean", replace 


** ------------------------------------------------
** ECDC data
** ------------------------------------------------
use "`datapath'\version02\2-working\paper02_ecdc", clear
...
** BMU - Bermuda
merge 1:1 iso date using "`datapath'\version02\2-working\paper02_jhopkins_clean"
sort iso date 
...
/// Latin America
replace pop = 5094114 if iso == "CRI" 
replace pop = 6486201 if iso == "SLV" 
replace pop = 17915567 if iso == "GTM" 
replace pop = 9904608 if iso == "HND" 
replace pop = 128932753 if iso == "MEX" 
replace pop = 6624554 if iso == "NIC" 
replace pop = 4314768 if iso == "PAN" 
replace pop = 45195777 if iso == "ARG" 
replace pop = 11673029 if iso == "BOL" 
replace pop = 212519409 if iso == "BRA" 
replace pop = 19116209 if iso == "CHL" 
replace pop = 50882884 if iso == "COL" 
replace pop = 17643060 if iso == "ECU" 
replace pop = 7132530 if iso == "PRY" 
replace pop = 32971846 if iso == "PER" 
replace pop = 3473727 if iso == "URY" 
replace pop = 28435943 if iso == "VEN"
...
save "`datapath'\version02\2-working\paper02_covid", replace
