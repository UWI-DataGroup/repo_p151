** HEADER -----------------------------------------------------
**  DO-FILE METADATA
    //  algorithm name					cdema_simulation_barbados.do
    ...
    ** Close any open log file and open a new log file
    capture log close
    log using "`logpath'\cdema_simulation_barbados", replace
** HEADER -----------------------------------------------------

** Load 100 SEIR simulations from CDC modelling

tempfile run01 run02 
import excel using "`datapath'/version01/1-input/cdc_simulation_BRB_28mar2020.xlsx", clear first sheet("run01")
rename Day day 
rename Sim#, lower
save `run01' 
import excel using "`datapath'/version01/1-input/cdc_simulation_BRB_28mar2020.xlsx", clear first sheet("run02")
rename Day day 
...
forval x = 1(1)100 {
    replace sim`x' = sim`x' * (`bbsize'/`artsize')
    } 
...
/*
    ** GRAPHIC NOT USED
    ** GRAPHIC HERE - ACUTE CARE 
    ** Hospital beds (HORIZONTAL STACKED BAR --> ACUTE CARE)
    #delimit ;
        graph hbar nhosp_p50_1 nhosp_p50_2 nhosp_p50_3 nhosp_p50_4
        , 
        stack 
        ...
        ylab(0(1000)7000, labs(4) nogrid glc(gs16))
        yscale(off) 
        legend(off)
        ;
    #delimit cr
    graph export "`outputpath'/report/stackedbar_acutecare.png", replace width(4000)
*/
restore 
    ...
    #delimit ;
        gr twoway 
            ...
            ysize(7.5) xsize(10)
                ...
                ;
        #delimit cr
graph export "`outputpath'/04_TechDocs/barbados_acutecare.png", replace width(4000)
    ...
    #delimit ;
        gr twoway 
            ...
            ysize(7.5) xsize(10)
                ...
                ;
        #delimit cr
graph export "`outputpath'/04_TechDocs/barbados_criticalcare.png", replace width(4000)
...
local acute_sf_p25_1 = -1*(peakhosp_p25_1 - 240)
local acute_sf_p75_1 = peakhosp_p75_1 - 240
local acute_sf_p25_2 = peakhosp_p25_2 - 240
local acute_sf_p75_2 = peakhosp_p75_2 - 240
local acute_sf_p25_3 = peakhosp_p25_3 - 240
local acute_sf_p75_3 = peakhosp_p75_3 - 240
** Critical Care Shortfall at Peak Demand for final paragraph 
local crit_sf_p25_1 = peakcrit_p25_1 - 40
local crit_sf_p75_1 = peakcrit_p75_1 - 40
local crit_sf_p25_2 = peakcrit_p25_2 - 40
local crit_sf_p75_2 = peakcrit_p75_2 - 40
local crit_sf_p25_3 = peakcrit_p25_3 - 40
local crit_sf_p75_3 = peakcrit_p75_3 - 40
...
putpdf text ("of the outbreak. The grey horizontal lines show the available beds. ")
...
putpdf table f1(1,1)=image("`outputpath'/04_TechDocs/barbados_acutecare.png")
putpdf table f1(2,1)=("(A) Acute Care Demand"), halign(center)
putpdf table f1(1,2)=image("`outputpath'/04_TechDocs/barbados_criticalcare.png")
...
putpdf table ta1(2,2)=("5.3%"), halign(center) border(top) border(bottom) border(left) border(right) 
putpdf table ta1(3,2)=("16.8%"), halign(center) border(top) border(bottom) border(left) border(right)
putpdf table ta1(4,2)=("40.9%"), halign(center) border(top) border(bottom) border(left) border(right)
putpdf table ta1(5,2)=("20.3%"), halign(center) border(top) border(bottom) border(left) border(right)
putpdf table ta1(6,2)=("5.2%"), halign(center) border(top) border(bottom) border(left) border(right)
putpdf table ta1(7,2)=("4.1%"), halign(center) border(top) border(bottom) border(left) border(right)
putpdf table ta1(8,2)=("2.9%"), halign(center) border(top) border(bottom) border(left) border(right)
putpdf table ta1(9,2)=("2.1%"), halign(center) border(top) border(bottom) border(left) border(right)
putpdf table ta1(10,2)=("2.3%"), halign(center) border(top) border(bottom) border(left) border(right)
...
** Save the PDF
putpdf save "`outputpath'/05_Outputs/covid19_report_BRB", replace
