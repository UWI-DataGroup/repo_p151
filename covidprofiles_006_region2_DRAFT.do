...
**Added on 27th April 2021: changing new cases per day to new cases in 3-day window
preserve 
    keep if mtype==1
    sort corder date 
    by corder: egen seq3 = seq(), f(1) b(3)
    egen cgroup = group(corder seq3)
    bysort cgroup: egen new3 = total(new)
    replace new3 = . if new3==0
    #delimit ;
        heatplot new3 i.corder date if mtype==1
        ,
...
** HEATMAP -- NEW DEATHS
** -----------------------------------------
**Added on 27th April 2021: changing new cases per day to new cases in 7-day window
*preserve 
    keep if mtype==3
    sort corder date 
    by corder: egen seq7 = seq(), f(1) b(7)
    egen cgroup = group(corder seq7)
    bysort cgroup: egen new7 = total(new)
    replace new7 = . if new7==0
    /// bysort cgroup: gen date7 = date if cgroup!=cgroup[_n - 1]
    /// replace date7 = date7[_n-1] if date7==.
    /// format date7 %tdNN/DD/CCYY

        #delimit ;
            heatplot new7 i.corder date if mtype==3
            ,
            color(RdYlBu , reverse intensify(0.75 ))
            /// cuts(@min(1){@max+1})
            cuts(5 15 25 35 45 55 65)
            keylabels(all, range(1))
            p(lcolor(white) lalign(center) lw(0.05))
            discrete
            statistic(asis)
